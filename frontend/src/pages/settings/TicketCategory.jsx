...
export default function TicketCategory({ ticketCategoryId, open, onClose, onSubmit, drawerSize = "large" }) {
    const [form] = Form.useForm();
    const pageLabel = Form.useWatch("tkc_label", form);

    const { submit, remove, loading } = useEntityForm({
        api: ticketCategoriesApi,
        entityId: ticketCategoryId,
        idField: "tkc_id",
        form,
        open,
    ...
    return (
        <Drawer
            title={pageLabel ? `Édition - ${pageLabel}` : "Nouvelle catégorie"}
            placement="right"
            onClose={handleClose}
            open={open}
            size={drawerSize}
        ...
        >
            <Spin spinning={loading} tip="Chargement...">
                <Form form={form} layout="vertical" onFinish={handleFormSubmit}>
                    <Form.Item name="tkc_id" hidden><Input /></Form.Item>
                    <Row gutter={[16, 8]}>
                        <Col span={24}>
                            <Form.Item
                                name="tkc_label"
